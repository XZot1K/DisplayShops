...
package xzot1k.plugins.ds.core.packets.v1_16_R1;
...
public class DPacket implements DisplayPacket {
    ...
    public DPacket(@NotNull DisplayShops pluginInstance, @NotNull Player player, @NotNull Shop shop, boolean showHolograms) {
        ...
        List<String> hologramFormat = shop.getShopItem() != null ? (shop.getOwnerUniqueId() == null ? this.getPluginInstance().getConfig().getStringList("admin-shop-format") :
                this.getPluginInstance().getConfig().getStringList("valid-item-format")) : (shop.getOwnerUniqueId() == null ? this.getPluginInstance().getConfig().getStringList("admin-invalid-item" +
        ...
        for (int i = hologramFormat.size(); --i >= 0; ) {
            ...
            if (line.contains("{buy-price}") && shop.getBuyPrice(true) < 0.0 || line.contains("{sell-price}") && shop.getSellPrice(true) < 0.0 || line.contains("{description}") && (shop.getDescription() == null || shop.getDescription().equalsIgnoreCase("")))
                continue;
            ...
            createStand(playerConnection, x, y, z, line.replace("{buy-price}", getPluginInstance().getManager().formatNumber(shop.getBuyPrice(true), true))
